...
(function() {
  ...
  var shell = jQuery('#elastic-search-shell');
  var trigger = jQuery('#elastic-search-trigger');

  function syncElasticSearchState() {
    var $input = jQuery(searchSelector);
    if ($input.val().trim().length > 0 || document.activeElement === $input.get(0)) {
      shell.addClass('is-expanded');
    } else {
      shell.removeClass('is-expanded');
    }
  }

  trigger.on('click', function() {
    shell.addClass('is-expanded');
    jQuery(searchSelector).trigger('focus');
  });

  jQuery(searchSelector).on('focus input blur', function() {
    setTimeout(syncElasticSearchState, 0);
  });
